...
macro (use_qt_module)
    # parse arguments
    set(OPTIONAL_ARGS ONLY_PLUGINS PLUGINS_OPTIONAL PLUGINS_LITERAL)
    ...
    foreach (PLUGIN ${ARGS_PLUGINS})
        if (ARGS_PLUGINS_LITERAL)
            set(PLUGIN_TARGET "${ARGS_PREFIX}::${PLUGIN}")
        else ()
        ...
        if (NOT TARGET "${PLUGIN_TARGET}")
            ...
            message(FATAL_ERROR "The ${ARGS_PREFIX}${ARGS_MODULE} package does not provide the target ${PLUGIN_TARGET}.")
        endif ()
        if ("${PLUGIN_TARGET}" IN_LIST "${ARGS_LIBRARIES_VARIABLE}")
            continue()
        endif ()
        set("${ARGS_LIBRARIES_VARIABLE}" "${${ARGS_LIBRARIES_VARIABLE}};${PLUGIN_TARGET}")
        message(STATUS "Linking ${META_TARGET_NAME} against Qt plugin ${PLUGIN_TARGET}.")
    endforeach ()
