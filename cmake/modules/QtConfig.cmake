...
if (BUILTIN_TRANSLATIONS AND (QM_FILES OR EXTERNAL_QM_FILES))
    ...
    if (EXISTS "${TRANSLATIONS_QRC_FILE}")
        message(STATUS "Using existing \"${TRANSLATIONS_QRC_FILE}\" for ${META_PROJECT_NAME}. "
...
function (configure_development_warnings_for_qt)
    set(OPTIONAL_ARGS)
    set(ONE_VALUE_ARGS TARGET)
    set(MULTI_VALUE_ARGS AUTOGEN_SOURCES)
    cmake_parse_arguments(ARGS "${OPTIONAL_ARGS}" "${ONE_VALUE_ARGS}" "${MULTI_VALUE_ARGS}" ${ARGN})

    if (NOT ENABLE_WARNINGS OR NOT (CMAKE_CXX_COMPILER_ID MATCHES ".*Clang" OR CMAKE_CXX_COMPILER_ID STREQUAL "GNU"))
        return() # no problematic warnings are configured anyway via configure_development_warnings()
    endif ()
    if (${QT_PACKAGE_PREFIX}Core_VERSION VERSION_LESS 6.11.0)
        return() # no problematic warnings to deal with before Qt 6.11.0
    endif ()

    # workaround sign conversion warning present in files generated by moc as of Qt 6.11.0
    list(APPEND COMPILE_OPTIONS_TO_CONFIGURE_FOR_AUTOGEN_SOURCES "-Wno-sign-conversion")

    # apply compiler options to relevant source files
    if (ARGS_TARGET)
        list(APPEND ARGS_AUTOGEN_SOURCES "${CMAKE_CURRENT_BINARY_DIR}/${ARGS_TARGET}_autogen/mocs_compilation.cpp")
    endif ()
    if (ARGS_AUTOGEN_SOURCES)
        message(STATUS "autogen src ${ARGS_TARGET}: ${ARGS_AUTOGEN_SOURCES}")
    endif ()
    set_property(
        SOURCE ${ARGS_AUTOGEN_SOURCES}
        APPEND
        PROPERTY COMPILE_OPTIONS "${COMPILE_OPTIONS_TO_CONFIGURE_FOR_AUTOGEN_SOURCES}")
endfunction ()
configure_development_warnings_for_qt(TARGET "${META_TARGET_NAME}" AUTOGEN_SOURCES "${GENERATED_DBUS_FILES}")
