                ...
                CppUtilities::walkThroughArchiveFromBuffer(
                    ...
                    [&foundExecutable, &error](std::string_view path, CppUtilities::ArchiveFile &&file) {
                        ...
                        if (currentExeInfo.isSymLink()) {
                            ...
                            const auto backupPath = currentExeInfo.path() % QChar('/') % currentExeInfo.baseName() % QStringLiteral("-backup-")
...
struct UpdateHandlerPrivate {
    ...
    QTimer timer;
    QSettings *settings;
    std::optional<CppUtilities::DateTime> lastUpdate;
};
    ...
    , m_p(std::make_unique<UpdateHandlerPrivate>())
{
    ...
    connect(&m_p->notifier, &UpdateNotifier::checkedForUpdate, this, &UpdateHandler::setLastUpdateCheckNow);
    applySettings();
}
...
CppUtilities::DateTime UpdateHandler::lastUpdateCheck() const
{
    if (m_p->lastUpdate.has_value()) {
        return m_p->lastUpdate.value();
    }
    m_p->settings->beginGroup(QStringLiteral("updating"));
    m_p->lastUpdate = CppUtilities::DateTime(m_p->settings->value("lastCheck").toULongLong());
    m_p->settings->endGroup();
    return m_p->lastUpdate.value();
}

void UpdateHandler::setLastUpdateCheck(CppUtilities::DateTime lastUpdate)
{
    m_p->settings->beginGroup(QStringLiteral("updating"));
    m_p->settings->setValue("lastCheck", static_cast<qulonglong>(lastUpdate.ticks()));
    m_p->settings->endGroup();
    m_p->lastUpdate = lastUpdate;
    emit lastUpdateCheckChanged(lastUpdate);
}
...
{
    const auto i = checkInterval();
    if (i < 0) {
        m_p->timer.stop();
    } else {
        m_p->timer.setInterval(i);
        if (!m_p->timer.isActive()) {
            m_p->timer.start();
        }
    }
}

void UpdateHandler::setLastUpdateCheckNow()
{
    setLastUpdateCheck(CppUtilities::DateTime::now());
}
...
QWidget *UpdateOptionPage::setupWidget()
{
    ...
    QObject::connect(m_p->updateHandler.notifier(), &UpdateNotifier::checkedForUpdate, widget, [this] { updateLatestVersion(); });
    return widget;
}

void UpdateOptionPage::updateLatestVersion()
{
    ui()->latestVersionValueLabel->setText(QCoreApplication::translate("QtUtilities::UpdateOptionPage", "%1 (last checked: %2)")
            .arg(m_p->updateHandler.notifier()->newVersion(), QString::fromStdString(m_p->updateHandler.lastUpdateCheck().toString())));
}
